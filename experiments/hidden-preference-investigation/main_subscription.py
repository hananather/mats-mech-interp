"""Hidden Preference Investigation - Subscription auth (Pro Max plan).

Same as main_with_sae.py but strips ANTHROPIC_API_KEY from the environment
so the Claude Agent SDK falls back to subscription auth via `claude login`.

Requires: `claude login` authenticated with a Pro Max account.
